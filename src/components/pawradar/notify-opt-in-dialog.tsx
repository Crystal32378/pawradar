 ...
 *   3. On tap: initialise SDK, ask for browser permission, get a subscription ID,
...
export function NotifyOptInDialog({
  ...
  useEffect(() => {
    if (!open) return;
    setUnsubscribeToken(localStorage.getItem(`pawradar:unsubscribe:${eventSlug}`));
  }, [eventSlug, open]);
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const subscriptionId = await promptForSubscription();
      if (!subscriptionId) {
      ...
      const res = await fetch('/api/subscriptions', {
        ...
        body: JSON.stringify({ subscriptionId, eventSlug }),
      });
      if (!res.ok) {
      ...
      if (data.unsubscribeToken) {
        localStorage.setItem(
          `pawradar:unsubscribe:${eventSlug}`,
  ...
  const handleUnsubscribe = async () => {
    ...
    try {
      ...
      localStorage.removeItem(`pawradar:unsubscribe:${eventSlug}`);
      setUnsubscribeToken(null);
      toast.success('已停止接收這場散步的異動通知');
      onOpenChange(false);
    } catch {
      toast.error('退訂失敗，請稍後再試');
    } finally {
      setSubscribing(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md rounded-3xl p-0 overflow-hidden">
        ...
        <div className="flex flex-col gap-2 p-6 pt-2">
          <Button
            onClick={unsubscribeToken ? handleUnsubscribe : handleSubscribe}
          ...
          >
            {subscribing ? (
              ...
              <>
                {unsubscribeToken ? <X size={16} /> : <Bell size={16} />}
                {unsubscribeToken ? '停止這場散步的通知' : '接收通知'}
              </>
            )}
          </Button>
          <Button
